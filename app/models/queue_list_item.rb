class QueueListItem < ApplicationRecord
  ...
  def self.list_of_time
    [ 1.minutes, 2.minutes, 10.minutes, 1.hours, 2.hours, 12.hours, 1.days, 1.months, 1.years ]
  end

  def set_time_of_appearance
    if self.index_of_time > QueueListItem.list_of_time.count
      self.destroy!
    elsif self.index_of_time > 0
      self.update!(time_of_appearance: Time.now + QueueListItem.list_of_time[index_of_time-1])
    end
  end
end
